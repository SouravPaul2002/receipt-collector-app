...
| [`frontend/components/common/WarrantyFormDrawer.tsx`](file:///c:/receipt-collector/frontend/components/common/WarrantyFormDrawer.tsx) | Reusable right-side slide-in drawer form wizard component for creating and editing warranties. | Features a wide right-side slide-in layout (`sm:580px md:680px lg:740px`), 2-step wizard flow (Step 1: Receipt & invoice document attachment to Google Drive Vault with file progress; Step 2: Form fields using Shadcn `<Field>`, `<FieldGroup>`, `<FieldLabel>`, `<Input>` with auto-computed expiry preview badge), and customizable title / initial data props. |
