...
| [`frontend/lib/api.ts`](file:///c:/receipt-collector/frontend/lib/api.ts) | Reusable type-safe API client utility. | Wraps native `fetch` with `credentials: 'include'` for cross-origin JWT cookie transmission, JSON headers, and generic `ApiResponse<T>` resolution. |
...
| [`frontend/components/ui/checkbox.tsx`](file:///c:/receipt-collector/frontend/components/ui/checkbox.tsx) | Shadcn Checkbox primitive component powered by `@base-ui/react/checkbox`. | Accessible styled checkbox for form inputs, terms & conditions, and multi-selection. |
| [`frontend/components/ui/radio-group.tsx`](file:///c:/receipt-collector/frontend/components/ui/radio-group.tsx) | Shadcn RadioGroup primitive component powered by `@base-ui/react/radio-group`. | Accessible radio group selection items for single-choice form inputs. |
| [`frontend/components/ui/alert-dialog.tsx`](file:///c:/receipt-collector/frontend/components/ui/alert-dialog.tsx) | Shadcn AlertDialog primitives (`AlertDialog`, `AlertDialogTrigger`, `AlertDialogContent`, `AlertDialogHeader`, `AlertDialogFooter`, `AlertDialogTitle`, `AlertDialogDescription`, `AlertDialogAction`, `AlertDialogCancel`) powered by `@base-ui/react/alert-dialog`. | Accessible modal confirmation dialog component for destructive operations like deleting warranties. |
| [`frontend/components/ui/sonner.tsx`](file:///c:/receipt-collector/frontend/components/ui/sonner.tsx) | Shadcn Sonner Toaster component. | Configures accessible rich toast notifications for successful additions, edits, and deletions across the application. |
| [`frontend/components/ui/progress.tsx`](file:///c:/receipt-collector/frontend/components/ui/progress.tsx) | Shadcn Progress bar primitive component. | Accessible animated progress bar for file scanning and upload states. |
